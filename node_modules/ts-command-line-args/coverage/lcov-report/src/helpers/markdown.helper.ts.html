...
<head>
    <title>Code coverage report for src/helpers/markdown.helper.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">src/helpers</a> markdown.helper.ts</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">82.86% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>145/175</span>
            </div>
...
export function createSection&lt;T&gt;(section: Content, config: UsageGuideConfig&lt;T&gt;): string {
    return `
${createHeading(section, config.parseOptions<span class="branch-0 cbranch-no" title="branch not covered" >?.d</span>efaultSectionHeaderLevel || 1)}
...
export function createSectionTable(rows: any[]): string {
    if (rows.length === 0) <span class="branch-0 cbranch-no" title="branch not covered" >{</span>
    ...
    return (
        ...
        (Array.isArray(groups) &amp;&amp;
            (groups.some((group) =&gt; group === option.group) ||
                (groups.some((group) =&gt; group === '_none')<span class="branch-0 cbranch-no" title="branch not covered" > &amp;&amp; option.group == null)</span>))
    );
}
&nbsp;
...
&nbsp;
export function createOptionRow(option: CommandLineOption, <span class="branch-0 cbranch-no" title="branch not covered" >includeAlias = true, <span class="branch-0 cbranch-no" title="branch not covered" >i</span>ncludeDescription = true): string {</span>
...
&nbsp;
    //TODO: add modifiers
&nbsp;
    const type = option.type ? option.type.name.toLowerCase() <span class="branch-0 cbranch-no" title="branch not covered" >: 'string';</span>
    const multiple = option.multiple || option.lazyMultiple ? '[]' : '';
&nbsp;
    return `${type}${multiple} `;
}
&nbsp;
<span class="cstat-no" title="statement not covered" ><span class="fstat-no" title="function not covered" >export function generateUsageGuides(args: IWriteMarkDown): string[] {</span></span>
<span class="cstat-no" title="statement not covered" >    function mapJsImports(imports: JsImport[], jsFile: string) {</span>
<span class="cstat-no" title="statement not covered" >        return [...imports, ...args.configImportName.map((importName) =&gt; ({ jsFile, importName }))];</span>
<span class="cstat-no" title="statement not covered" >    }</span>
<span class="cstat-no" title="statement not covered" ></span>
<span class="cstat-no" title="statement not covered" >    return args.jsFile</span>
<span class="cstat-no" title="statement not covered" >        .reduce(mapJsImports, new Array&lt;JsImport&gt;())</span>
<span class="cstat-no" title="statement not covered" >        .map(({ jsFile, importName }) =&gt; loadArgConfig(jsFile, importName))</span>
<span class="cstat-no" title="statement not covered" >        .filter(isDefined)</span>
<span class="cstat-no" title="statement not covered" >        .map(createUsageGuide);</span>
<span class="cstat-no" title="statement not covered" >}</span>
&nbsp;
<span class="cstat-no" title="statement not covered" ><span class="fstat-no" title="function not covered" >export function loadArgConfig(jsFile: string, importName: string): UsageGuideConfig | undefined {</span></span>
<span class="cstat-no" title="statement not covered" >    const jsPath = join(process.cwd(), jsFile);</span>
<span class="cstat-no" title="statement not covered" >    // eslint-disable-next-line @typescript-eslint/no-var-requires</span>
<span class="cstat-no" title="statement not covered" >    const jsExports = require(jsPath);</span>
<span class="cstat-no" title="statement not covered" ></span>
<span class="cstat-no" title="statement not covered" >    const argConfig: UsageGuideConfig = jsExports[importName];</span>
<span class="cstat-no" title="statement not covered" ></span>
<span class="cstat-no" title="statement not covered" >    if (argConfig == null) {</span>
<span class="cstat-no" title="statement not covered" >        console.warn(`Could not import ArgumentConfig named '${importName}' from jsFile '${jsFile}'`);</span>
<span class="cstat-no" title="statement not covered" >        return undefined;</span>
<span class="cstat-no" title="statement not covered" >    }</span>
<span class="cstat-no" title="statement not covered" ></span>
<span class="cstat-no" title="statement not covered" >    return argConfig;</span>
<span class="cstat-no" title="statement not covered" >}</span>
&nbsp;
<span class="cstat-no" title="statement not covered" ><span class="fstat-no" title="function not covered" >function isDefined&lt;T&gt;(value: T | undefined | null): value is T {</span></span>
...
&nbsp;</pre></td></tr></table></pre>
            ...
            <div class='footer quiet pad2 space-top1 center small'>
                ...
                at Mon May 09 2022 08:08:35 GMT+0000 (Coordinated Universal Time)
            </div>
        </div>
